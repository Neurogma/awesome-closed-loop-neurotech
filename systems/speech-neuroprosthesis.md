...
**Maturity: Clinically validated.** This recipe describes systems demonstrated in published clinical trials with implanted participants (Willett et al., Nature 2023; Card et al., NEJM 2024). It is *not* an approved medical device. The "clinically validated" label refers to demonstration under trial conditions, not regulatory clearance.
...
| Feedback | The participant hears/reads their own decoded output in real time | This allows the participant to correct and adapt; it is part of the human-in-the-loop adaptation process. |
...
## Validation strategy
Validation uses published word-error-rate on held-out sessions with the same participant, with cross-session evaluation increasingly used to test decoder stability over time (see the LINK dataset). Offline reproduction of the RNN decoder against publicly released data (via `speechBCI` and `nejm-brain-to-text`) is also possible.
